...
names(auto_size) <- paste("trio", 1:6, sep = "")
...
# trio3 and trio6 have parental mismatch, we must remove them - we do not know the true mother/father

dncAD0_noGQAB <- candidates_DP %>% filter_gq(., 20) %>% filter_bam %>% filter_rel
dncAD0_noGQAB <- dncAD0_noGQAB[!names(dncAD0_noGQAB) %in% c("trio3", "trio6")]
...
callability <- callability_M[["AD0"]][["35"]][-c(3,6),]
auto_size_bytrio <- auto_size[-c(3,6)]
