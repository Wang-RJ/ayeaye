...
trio_table$trio <- paste("trio", 1:19, sep = "")
...
maxdepths <- read.csv("wu_maxdepths.csv", header = FALSE, check.names = FALSE)
...
# par(mfrow = c(4,5)); for(i in 1:19) { hist(as.numeric(unlist(dpgq_gq[[i]])), main = names(candidates_DP_GQ)[[i]], xlab = "GQ score") }
# par(mfrow = c(4,5)); for(i in 1:19) { hist(as.numeric(unlist(dpgq_depths[[i]]) %>% .[. < 80]), breaks = 20, main = names(candidates_DP_GQ)[[i]], xlab = "depth < 80") }
# par(mfrow = c(4,5)); for(i in 1:19) { hist(as.numeric(dpgq_allelicbalance[[i]]), breaks = 15, xlim = c(0, 1), main = names(candidates_DP_GQ)[[i]], xlab = "allelic balance (child)") }
...
plot_GQ <- function(AB_cutoff) {
 ...
 for(GQ in seq(20,80,20)) {
   ...
   hist(tmp_ab, xlab = "allelic balance", ylim = c(0,500), xlim = c(0.1,0.8), breaks = 20,
...
neighbors <- extract_neighbors(candidates_DP_GQ60_BAM_REL_AB, 150)
