# Must be in baboon/Wuetal/
# Process bam files to remove likely GATK remapping errors

rawbam_bcfcalls <- read.table("vcf_files/bcfpileup_wubaboon_merged.decap.vcf", comment.char = "", header = FALSE, check.names = FALSE)
rawbam_header <- read.table("vcf_files/bcfpileup_wubaboon_merged.header", comment.char = "", check.names = FALSE)
