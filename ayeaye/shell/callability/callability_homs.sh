...
bcftools view -Ou -M2 -m2 -r $(tr '\n' ',' < candidate_contigs.txt) -i'INFO/AC < 2' -s $sample ../vcf_filesBGI/8/output.filtered.snps.removed.CalculateGenotypePosteriors.deNovo.vcf.gz | \
bcftools annotate -i'(GT[0]="RR" && GT[1]="RR")' | awk 'BEGIN {srand()} (rand() <= 1 || /^#/ )' | bcftools view -Oz -o homs/homcandidates.trio$i.vcf.gz
done
...
# bcftools mpileup -Oz -A -f /N/project/phi_hahnlab-phi/baboon/reference/GCA_000264685.2_Panu_3.0_genomic.fna -Q 10 \
# -R <(bcftools query homs/homxmitDP.trio$i.vcf.gz -f'%CHROM\t%POS\n') \
# -a ADF,ADR,AD,DP /N/project/phi_hahnlab-phi/baboon/bam_files/$childbam > homs/pileup/hompileup.trio$i.vcf.gz &
# done
...
for i in {1..12}; do bcftools index homs/homtransmitted.trio$i.vcf.gz; done
for i in {1..12}; do zgrep -v '#' homs/homdenom.trio$i.vcf.gz | wc -l >> homs/columns/hom_denom.column; done
for i in {1..12}; do zgrep -v '#' homs/homtransmitted.trio$i.vcf.gz | wc -l >> homs/columns/hom_transmit.column; done
for i in {1..12}; do zgrep -v '#' homs/homxmitDP.trio$i.vcf.gz | wc -l >> homs/columns/hom_DP.column; done
